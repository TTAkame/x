...
{
    const float bias = 0.0005; // constant bias to prevent shadow acne

    // Transform the shadow coordinates from clip space to texture space
    vec3 shadow_uv = shadow_coord.xyz / shadow_coord.w;
    shadow_uv = shadow_uv * 0.5 + 0.5;

    // Retrieve the depth of the closest fragment from the light's perspective
    float closest_depth = texture(shadow_tex, shadow_uv.xy).r;
